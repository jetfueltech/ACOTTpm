...
const TransactionForm: React.FC<{ 
  ...
  return (
    <form onSubmit={handleSubmit} className="p-6 bg-white rounded-xl border border-neutral-300 space-y-6 mb-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <TransactionFormInputField label="Type" id="type" required value={type} onChange={e => setType((e.target as HTMLSelectElement).value as TransactionType)}>
            <select id="type" value={type} onChange={e => setType(e.target.value as TransactionType)} className="mt-1 block w-full px-3 py-2 bg-white text-neutral-900 border border-neutral-300 rounded-md shadow-sm focus:outline-none focus:ring-primary focus:border-primary sm:text-sm">
        ...
        <TransactionFormInputField label="Property (Optional)" id="propertyId" value={propertyId} onChange={e => setPropertyId((e.target as HTMLSelectElement).value)}>
            <select id="propertyId" value={propertyId} onChange={e => setPropertyId(e.target.value)} className="mt-1 block w-full px-3 py-2 bg-white text-neutral-900 border border-neutral-300 rounded-md shadow-sm focus:outline-none focus:ring-primary focus:border-primary sm:text-sm">
      ...
      <div className="flex justify-end space-x-3">
        <button type="button" onClick={onClose} className="px-4 py-2 border border-neutral-300 rounded-md text-sm font-medium text-neutral-700 hover:bg-neutral-50">Cancel</button>
        <button type="submit" className="px-6 py-2 bg-primary text-white rounded-md text-sm font-medium hover:bg-primary-dark">Add Transaction</button>
      </div>
    </form>
  );
};
  ...
  return (
    <div className="space-y-8">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <h2 className="text-3xl font-semibold text-neutral-800">Financials</h2>
        <button
        ...
        >
          ...
          <span className={`ml-1 transform transition-transform ${showForm ? 'rotate-180' : ''}`} role="img" aria-label="Toggle form">🔽</span>
        </button>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className={`${netProfit >= 0 ? 'bg-blue-50 border-blue-200' : 'bg-orange-50 border-orange-200'} p-6 rounded-xl border`}>
          <p className={`text-sm font-medium ${netProfit >= 0 ? 'text-blue-700' : 'text-orange-700'}`}>Net Profit/Loss</p>
          <p className={`text-2xl font-bold ${netProfit >= 0 ? 'text-blue-600' : 'text-orange-600'}`}>{formatCurrency(netProfit)}</p>
        </div>
      </div>
